...
- `patches/`: three Slotstream source patches, applied in alphabetical order: retry wording + server-side failure logging, then prefix retention (`--prefix-cache-tokens n|full` on `RuntimeAllocationPolicy`, charged against the pool), then the stale-pressure cross-check (`RequestPressurePolicy`). The prefix-retention patch is unbuilt as of 2026-09-11; `scripts/slotkeeper patch --status` shows which are in the installed binary. Regenerate an incremental patch with `diff -ruN -x .build -x lib -x '*.orig' <before> <after>` and rewrite the path prefixes to `a/` and `b/`.
