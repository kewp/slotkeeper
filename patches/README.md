...
2026-09-12 08:41: installed as `slotstream-0.2.14-local-20260912084038` with the
window at 49,152 tokens (plan: 20 experts/layer, 11.7 GB peak). A real session
had just died at the 65,536 window with "the configured context no longer fits
current availability" while the machine reported normal pressure and 54% free:
the governor could not form a feasible plan for the window plus a full held
conversation, and refused everything. It now sheds the conversation first.
