...
| --- | --- | --- |
| Total memory target | `SLOTSTREAM_MEMORY_GB` → `--memory-gb` | From the working set downward until one serves a prompt without a memory failure or critical pressure. Overrides auto's own 33 GB model ceiling, so a large machine is measured at what it has. |
